...
def prepare_data(dataset):
    ...
    data.sort(key=lambda x: int(x[-8:-6]))  ##pet 与mr
    return data
...
for idx_num in range(15):
  ...
  while(num_epoch==idx_num):
      ...
      with tf.Session() as sess:
          ...
          sess.run(init_op)
          # data_ir=prepare_data('img_A')##数据集文件夹
          # data_vi=prepare_data('img_B')

          data_ir=prepare_data('nature/a1')
          data_vi=prepare_data('nature/a2')
          for i in range(len(data_ir)):
              ...
              if i<=9:
                  ...
                  image_path = os.path.join(image_path, data_ir[i][-9:-4] + ".png")
              else:
