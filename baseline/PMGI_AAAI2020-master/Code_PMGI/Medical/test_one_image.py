...
def prepare_data(dataset):
    ...
    # data.sort(key=lambda x:int(x[len(data_dir)+1:-6]))
    # data.sort(key=lambda x:int(x[-7:-6]))##ct与mr
    data.sort(key=lambda x:int(x[-7:-4]))##pet 与mr
...
for idx_num in range(35):
  ...
  while(num_epoch==idx_num):
      ...
      with tf.Session() as sess:
          ...
          sess.run(init_op)
          data_ir=prepare_data('pet-mri/mri') ##准备数据集
          data_vi=prepare_data('pet-mri/pet_I')
          for i in range(len(data_ir)):
              start=time.time()
              train_data_ir,train_data_vi=input_setup(i,data_ir,data_vi)
              ...
              # image_path = './result'
              image_path = './pet-mri/fusion'
              if not os.path.exists(image_path):
                  os.makedirs(image_path)
              if i<=9:
                  image_path = os.path.join(image_path,data_ir[i][-7:-4]+".png")
              else:
                  image_path = os.path.join(image_path,data_ir[i][-7:-4]+".png")
              end=time.time()
              # print(out.shape)
